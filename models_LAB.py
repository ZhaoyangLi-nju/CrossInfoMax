...
class Encoder(nn.Module):
    ...
    def __init__(self, encoder='resnet18', pretrained='imagenet',in_channel=1):
        ...
        self.up_l_28 = nn.Sequential(
            nn.Conv2d(dims[2], 3, 3, 1, 1, bias=False),
            nn.Tanh()
        )
    ...
    def forward(self, x, no_grad=False):
        ...
        if no_grad:
            ...
            LAB_gen.append(l_224)
            LAB_gen.append(l_112)
            LAB_gen.append(l_56)
            LAB_gen.append(l_28)
            LAB_gen.append(l_14)
            out['LAB'] = LAB_gen
...
class Contrastive_CrossModal_Conc(nn.Module):
    ...
    def __init__(self, cfg, alpha=0.5, beta=1.0, gamma=0.05, feat_channel=128, feat_size=8, device=None):
        ...
        self.local_l_cross = LocalDiscriminator(6)  # + 128
    ...
    def forward(self, rgb, lab=None, label=None, class_only=False):
        res_dict = {}
        if class_only:
        ...
        LOCAL_cross_lab = torch.zeros(1).to(self.device)


        for i, (LAB_gen,LAB_label) in enumerate(zip(result_rgb['LAB'],lab)):

            if i + 1 > self.cfg.MULTI_SCALE_NUM:
                break

            cross_pos = torch.cat((LAB_gen, LAB_label), 1)
            cross_neg = torch.cat((LAB_gen, torch.cat((LAB_label[1:], LAB_label[0].unsqueeze(0)), dim=0)), 1)
            Ej = -F.softplus(-self.local_l_cross(cross_pos)).mean()
            Em = F.softplus(self.local_l_cross(cross_neg)).mean()
            LOCAL_cross_lab += (Em - Ej) * self.beta
        ...
        return {'gen_lab_loss': sum(LOCAL_cross_lab), 'local_rgb_loss': LOCAL_RGB, 
                'prior_rgb_loss': PRIOR_RGB, 'lab_gen': result_rgb['LAB']}
